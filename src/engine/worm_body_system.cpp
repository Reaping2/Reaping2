...
void WormBodySystem::Update(double DeltaTime)
{
    for (auto actor : mScene.GetActorsFromMap( GetType_static() ))
    {
        ...
        if (followed.IsValid()) // not dead yet
        {
            ...
            double const maxDist = (followedCollisionC->GetRadius() + collisionC->GetRadius())*wormBodyC->GetDistancePercent();
            if (wormBodyC->GetPrevPositions().size()>maxDist)
            {
                wormBodyC->GetPrevPositions().pop_front();
            }
