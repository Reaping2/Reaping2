...
ModelValue::~ModelValue()
{
	...
	case Mt_DoubleFunction:
		delete (double_function_t*)mValue.f; break;
	case Mt_IntFunction:
		delete (int_function_t*)mValue.f; break;
	case Mt_StringFunction:
		delete (string_function_t*)mValue.f; break;
	case Mt_VoidFunction:
		delete (void_function_t*)mValue.f; break;
	default:
		break;
	}
}
...
{
	if(mType==Mt_Int) return *(mValue.i);
...
{
	if(mType==Mt_Double) return *(mValue.d);
	OBSERVABLE_ASSERT(false);
	return 0;
}
...
{
	if(mType==Mt_VoidFunction&&mValue.f) return (*(void_function_t*)mValue.f)();
	OBSERVABLE_ASSERT(false);
}

void ModelValue::operator()(int32_t Arg) const
{
	if(mType==Mt_IntFunction&&mValue.f) return (*(int_function_t*)mValue.f)(Arg);
	else if(mType==Mt_DoubleFunction&&mValue.f) return (*(double_function_t*)mValue.f)((double)Arg);
	OBSERVABLE_ASSERT(false);
}

void ModelValue::operator()(double Arg) const
{
	if(mType==Mt_DoubleFunction&&mValue.f) return (*(double_function_t*)mValue.f)(Arg);
	else if(mType==Mt_IntFunction&&mValue.f) return (*(int_function_t*)mValue.f)((int32_t)Arg);
	OBSERVABLE_ASSERT(false);
}

void ModelValue::operator()(std::string const& Arg) const
{
	if(mType==Mt_StringFunction&&mValue.f) return (*(string_function_t*)mValue.f)(Arg);
	OBSERVABLE_ASSERT(false);
}
